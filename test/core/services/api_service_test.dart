...
void main() {
  ...
  group('ApiService — getErrorMessage', () {
    ...
    test('TimeoutException returns timeout message', () {
      expect(ApiService.getErrorMessage(TimeoutException('slow')), 'Request timeout.');
    });
    ...
    test('BadRequestException returns its message', () {
      expect(ApiService.getErrorMessage(BadRequestException('bad input')), 'bad input');
    });

    test('UnauthorizedException returns its message', () {
      expect(ApiService.getErrorMessage(UnauthorizedException('no auth')), 'no auth');
    });

    test('ForbiddenException returns its message', () {
      expect(ApiService.getErrorMessage(ForbiddenException('no access')), 'no access');
    });

    test('NotFoundException returns its message', () {
      expect(ApiService.getErrorMessage(NotFoundException('missing')), 'missing');
    });

    test('RateLimitException returns its message', () {
      expect(ApiService.getErrorMessage(RateLimitException('too many')), 'too many');
    });

    test('ServerException returns its message', () {
      expect(ApiService.getErrorMessage(ServerException('server down')), 'server down');
    });

    test('EmailVerificationRequiredException returns its message', () {
      expect(
        ApiService.getErrorMessage(EmailVerificationRequiredException('verify email')),
        'verify email',
      );
    });
  ...
  group('ApiService — _handleResponse and _expectMapResponse via FakeApiService', () {
    ...
    test('200 with non-map JSON throws FormatException', () async {
      ...
      await expectLater(() => fakeApi.get('/test'), throwsA(isA<FormatException>()));
    });
    ...
    test('400 throws BadRequestException', () async {
      ...
      await expectLater(() => fakeApi.get('/test'), throwsA(isA<BadRequestException>()));
    });

    test('400 with detail field extracts specific message', () async {
      fakeApi.getStubs['/test'] =
          http.Response('{"detail":"custom error"}', 400);
      await expectLater(
        () => fakeApi.get('/test'),
        throwsA(
    ...
    test('401 throws UnauthorizedException', () async {
      ...
      await expectLater(() => fakeApi.get('/test'), throwsA(isA<UnauthorizedException>()));
    });

    test('403 throws ForbiddenException', () async {
      fakeApi.getStubs['/test'] = http.Response('forbidden', 403);
      await expectLater(() => fakeApi.get('/test'), throwsA(isA<ForbiddenException>()));
    });

    test('404 throws NotFoundException', () async {
      fakeApi.getStubs['/test'] = http.Response('not found', 404);
      await expectLater(() => fakeApi.get('/test'), throwsA(isA<NotFoundException>()));
    });
    ...
    test('429 throws RateLimitException', () async {
      ...
      await expectLater(() => fakeApi.get('/test'), throwsA(isA<RateLimitException>()));
    });

    test('500 throws ServerException', () async {
      fakeApi.getStubs['/test'] = http.Response('server error', 500);
      await expectLater(() => fakeApi.get('/test'), throwsA(isA<ServerException>()));
    });
    ...
    test('418 throws generic ApiException for unknown status', () async {
      ...
      await expectLater(() => fakeApi.get('/test'), throwsA(isA<ApiException>()));
    });
  });

  group('ApiService — _requestResponse via MockClient (requiresAuth: false)', () {
    test('GET 200 returns response', () async {
      ...
      final response = await service.getResponse('/test', requiresAuth: false);
    ...
    test('POST with Map body encodes JSON', () async {
      ...
      final service = ApiService(client: client);
      await service.postResponse('/test', body: {'key': 'value'}, requiresAuth: false);
    ...
    test('POST with null body returns response', () async {
      ...
      final service = ApiService(client: client);
      final response =
          await service.postResponse('/test', body: null, requiresAuth: false);
      expect(response.statusCode, 200);
    });
    ...
    test('PUT 200 returns response', () async {
      ...
      final service = ApiService(client: client);
      final response =
          await service.putResponse('/test', body: {'x': 1}, requiresAuth: false);
      expect(response.statusCode, 200);
    });
    ...
    test('DELETE 200 returns response', () async {
      ...
      final service = ApiService(client: client);
      final response =
          await service.deleteResponse('/test', requiresAuth: false);
      expect(response.statusCode, 200);
    });
