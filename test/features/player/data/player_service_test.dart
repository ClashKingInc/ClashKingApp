...
Map<String, dynamic> _playerBasicJson(
  String tag, {
  String name = 'Hero',
  String clanTag = '#CLAN1',
}) => <String, dynamic>{
  'tag': tag,
  'name': name,
  'trophies': 1000,
...
void main() {
  ...
  group('PlayerService — initPlayerData (errors)', () {
    ...
    test('resets isLoading to false on network exception', () async {
      final service = PlayerService(apiService: NetworkErrorApiService());
      await service.initPlayerData(['#ABC']);
      expect(service.isLoading, isFalse);
    });
    ...
    test('throws 503 when throwOnError = true', () async {
      final fakeApi = FakeApiService();
      fakeApi.postStubs['/players'] = http.Response('maintenance', 503);
      final service = PlayerService(apiService: fakeApi);
      await expectLater(
        () => service.initPlayerData(['#ABC'], throwOnError: true),
        throwsA(anything),
  ...
  group('PlayerService — getPlayerAndClanData', () {
    test(
      'merges fetched player into existing profiles without replacing',
      () async {
        final fakeApi = FakeApiService();
        fakeApi.postStubs['/players'] = http.Response(
          jsonEncode({
            'items': [_playerBasicJson('#NEW', name: 'Newcomer')],
          }),
          200,
        );
        final service = PlayerService(apiService: fakeApi);
        service.processBulkPlayerData([], [
          _playerBasicJson('#ORIG', name: 'Original'),
        ]);
        expect(service.profiles, hasLength(1));
    ...
    test('loads player data through the official proxy path', () async {
      final fakeApi = FakeApiService();
      fakeApi.postStubs['/players'] = http.Response(
        jsonEncode({
  ...
  group('PlayerService — loadPlayerData (200 with items)', () {
    test('refreshes existing profiles on 200', () async {
      final fakeApi = FakeApiService();
      fakeApi.postStubs['/players'] = http.Response(
        jsonEncode({
          'items': [_playerBasicJson('#P1', name: 'Extended Alice')],
        }),
        200,
      );
      final service = PlayerService(apiService: fakeApi);
  ...
  group('PlayerService — loadPlayerWarStats', () {
    ...
    test('does not throw on non-200 when throwOnError = false', () async {
      final fakeApi = FakeApiService();
      fakeApi.postStubs['/war/players/warhits'] = http.Response('error', 500);
      final service = PlayerService(apiService: fakeApi);
      await expectLater(
  ...
  group('PlayerService — linkClansToPlayer', () {
    ...
    test('links matching clan to player profile', () async {
      ...
      fakeApi.postStubs['/players'] = http.Response(
        jsonEncode({
          'items': [_playerBasicJson('#P1', clanTag: '#CLAN1')],
        }),
        200,
      );
      final service = PlayerService(apiService: fakeApi);
      ...
      final clanService = ClanService(apiService: fakeClanApi);
