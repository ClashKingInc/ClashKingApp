...
> Moteur : Chromium headless, Expo web (semantics tree activé).
...
| 1.2 | `#root` monté (Expo initialisé)                  | ✅     |
...
| #    | Test                                                                     | Status |
| ---- | ------------------------------------------------------------------------ | ------ |
| 2.1  | Page de login visible (#root [role], #root input, #root button présents) | ✅     |
...
| 7.11 | Confirmer → charge les données → redirige vers authenticated shell 🎮 | ✅     |

---

## §8 — Session — Déconnexion

**Fichier** : `logout.spec.ts`, `settings.spec.ts` | **Projets** : `chromium-auth`, puis `chromium-auth-logout` 🔑

| #   | Test                                                                     | Status |
| --- | ------------------------------------------------------------------------ | ------ |
| 8.1 | Bouton "Log out" accessible depuis account setup                         | ✅     |
| 8.2 | Clic "Log out" → retour login (onglets Discord/Email)                    | ✅     |
| 8.3 | Après logout → bouton Login visible (session effacée)                    | ✅     |
| 8.4 | Logout depuis Settings (authenticated shell) → dialog de confirmation 🎮 | ✅     |
| 8.5 | Confirmer logout dans dialog → retour login 🎮                           | ✅     |
| 8.6 | Annuler logout dans dialog → reste dans l'app 🎮                         | ✅     |

---

## §9 — Home (page principale)

**Fichier** : `dashboard_content.spec.ts` | **Projet** : `chromium-auth` 🔑 🎮

| #    | Test                                                       | Status |
| ---- | ---------------------------------------------------------- | ------ |
| 9.1  | Primary navigation visible (Home, Clan, War)               | ✅     |
| 9.2  | Account selector pill visible dans l'app bar               | ✅     |
| 9.3  | Home affiche plus de 10 éléments semantics (content cards) | ✅     |
| 9.4  | Switch Clan tab + retour Home ne crash pas                 | ✅     |
...
| #     | Test                                                | Status |
| ----- | --------------------------------------------------- | ------ |
| 11.1  | Onglet Clan accessible depuis la primary navigation | ✅     |
...
| #     | Test                                               | Status |
| ----- | -------------------------------------------------- | ------ |
| 12.1  | Onglet War accessible depuis la primary navigation | ✅     |
...
| #     | Test                                                                                         | Status |
| ----- | -------------------------------------------------------------------------------------------- | ------ |
| 14.1  | Page Settings accessible via avatar (authenticated shell)                                    | ✅     |
...
| 14.11 | Confirmer logout → retour login (couvert après les autres specs dans `chromium-auth-logout`) | ✅     |
| 14.12 | Tap FAQ → FaqScreen s'ouvre                                                                  | ✅     |
| 14.13 | Tap Notifications → NotificationSettingsPage s'ouvre                                         | ✅     |

---

## §15 — Gestion des comptes CoC (depuis authenticated shell)
...
| 15.7 | Retour depuis manage → retour authenticated shell  | ✅     |
