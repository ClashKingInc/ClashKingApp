...
// Tests for the War page (third tab in the primary navigation, label "War").
...
//
// Detection note: Expo renders nav labels, card titles and tab labels as
// #root [role], #root input, #root button *textContent*, not [aria-label], so we target by text.
...
test.describe("War / CWL page", () => {
  ...
  test("War tab is accessible from the primary navigation", async ({
    page,
  }) => {
  ...
  test("War page loads without crashing", async ({ page }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
    if (!(await isOnAuthenticatedShell(page)))
      test.skip(true, "No CoC accounts — not on authenticated shell");
  ...
  test('"No clan" state shows correct message on War page', async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
  ...
  test("War History card is visible when the clan has war logs", async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
  ...
  test("Active war card shows war state (preparation / in war / ended)", async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
  ...
  test("tapping war card opens War detail screen", async ({ page }) => {
    ...
    // WarCard is a Pressable — tap at ~180px from top, center of page
    const warEl = page
      .getByText(/Preparation/i)
      .or(page.getByText(/War ended/i));
  ...
  test("tapping War History card opens War Stats screen", async ({ page }) => {
    ...
    if ((await warHistoryEl.count()) === 0)
      test.skip(true, "War History card not present");
  ...
  // Navigation path: War → War History → first war log entry → WarScreen.
  // This path is always available (clan has war history) unlike an active war card.
  test("war detail screen shows Statistics, Events and Teams tabs", async ({
    page,
  }) => {
    ...
    if ((await warLogTab.count()) === 0)
      test.skip(true, "War Log tab not found on ClanWarStatsScreen");

    // Step 3: click the first war entry in the log to open WarScreen.
    // Entries are rendered as Pressables — tap the first "War ended" / "Victory" / "Defeat" text.
    ...
    if ((await firstEntry.count()) === 0)
      test.skip(true, "No war log entries found");
    ...
    await expect(
      page.getByText("Statistics", { exact: true }).first(),
    ...
    const tabBar = page
      ...
      .first();
    await tabBar
      .evaluate((el: Element) => (el.scrollLeft = 999))
      .catch(() => {});
    await page.waitForTimeout(300);
    await expect(page.getByText("Teams", { exact: true }).first()).toBeAttached(
      { timeout: 5_000 },
    );
  });

  // §12.10 — CWL detail screen tabs (Rounds / Teams / Members)
  test("CWL detail screen shows Rounds, Teams and Members tabs", async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
    ...
    if ((await cwlEl.count()) === 0)
      test.skip(true, "Not in CWL season — CWL card not present");
    ...
    await expect(
      page.getByText("Rounds", { exact: true }).first(),
  ...
  test("pull-to-refresh on War page does not crash the app", async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
