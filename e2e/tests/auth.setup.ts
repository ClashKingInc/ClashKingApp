...
const TEMPORARY_ACCOUNT_FILE = path.join(
  __dirname,
  "../playwright/.auth/temporary-account",
);
...
setup("authenticate with email", async () => {
  ...
  const forceTemporaryAccount = process.env.E2E_PROVISION_ACCOUNT === "true";
  setup.skip(
  ...
  // ── Direct API login (bypass Expo form to avoid keyboard-layout issues) ─
  // pressSequentially('@') sends the wrong character on AZERTY layouts,
  // making the Expo form unreliable. A direct POST is faster and correct.
  ...
  let capturedCookies: Awaited<
    ReturnType<typeof apiContext.storageState>
  >["cookies"] = [];
  ...
  try {
    ...
    if (email && password && !forceTemporaryAccount) {
    ...
    if (!capturedAccessToken && canProvisionAccount) {
      ...
      if (!register.ok()) {
        throw new Error(
          `temporary account registration returned ${register.status()}`,
        );
      }
      ...
      const code =
        developmentCode ??
        (await waitForOtp(provisionedEmail, { notBefore: requestedAt }));
      ...
      if (!verify.ok()) {
        throw new Error(
          `temporary account verification returned ${verify.status()}`,
        );
      }
      ...
      if (typeof json?.access_token === "string" && json.access_token) {
        ...
        console.log(
          "[auth.setup] temporary verified account tokens captured ✓",
        );
      }
    }
    if (capturedAccessToken) {
      ...
      if (capturedCookies.length === 0) {
        throw new Error(
          "web authentication did not issue a browser session cookie",
        );
      }
    }
  } catch (err) {
  ...
  if (!capturedAccessToken) {
    console.warn(
      "[auth.setup] tokens not captured — chromium-auth tests will run with empty auth",
    );
      ...
      origins: [
        {
          ...
          localStorage: [
            {
              name: "e2e.access_token",
              value: JSON.stringify(capturedAccessToken),
            },
          ],
        },
      ],
  ...
  if (temporaryAccountCreated)
    fs.writeFileSync(TEMPORARY_ACCOUNT_FILE, "created-by-playwright");
