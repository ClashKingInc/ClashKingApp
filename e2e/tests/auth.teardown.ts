...
teardown("delete temporary email account", async () => {
  teardown.skip(
    !fs.existsSync(TEMPORARY_ACCOUNT_FILE),
    "No temporary E2E account was created",
  );
  ...
  const storedToken = state.origins
    ...
    .find((entry) => entry.name === "e2e.access_token")?.value;
  if (!storedToken)
    throw new Error("Temporary E2E account has no stored access token");
  ...
  try {
    ...
    expect(
      response.ok(),
      `account cleanup returned ${response.status()}${responseBody}`,
    ).toBe(true);
