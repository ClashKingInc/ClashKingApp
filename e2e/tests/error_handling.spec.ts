...
//
// IMPORTANT — Expo web semantics: when the accessibility tree is enabled,
// Expo renders <#root [role], #root input, #root button> elements whose *textContent* carries the UI
...
// Blocks API, loads the app, waits for Expo to boot and attempt startup.
// Blocks the configured API so that the startup widget cannot complete
// initialization and renders ErrorPage instead.
//
...
async function loadWithApiBlocked(page: any): Promise<boolean> {
  ...
  return hasExpoAccessibility(page);
}

// Locator for the full-screen ErrorPage title ("Connection Problem" for network
// errors, "Oops" for generic). Matches the leaf #root [role], #root input, #root button whose text is the
...
// The primary navigation "Home" label is present on a normal dashboard (where a
...
async function isOnErrorPage(page: any, stable = false): Promise<boolean> {
  const check = async () => {
    const hasError =
      (await page.getByText(/Connection Problem|Oops/i).count()) > 0;
...
test.describe("Error handling — API unreachable", () => {
  test('"Connection Problem" page appears when API is unreachable at startup', async ({
    page,
  }) => {
    test.setTimeout(60_000);
    const semanticsOk = await loadWithApiBlocked(page);
    if (!semanticsOk) test.skip(true, "Expo semantics unavailable");
    if (!(await isOnErrorPage(page))) {
      test.skip(
        true,
        "Error page not triggered (startup completed or different error path)",
      );
    }
  ...
  test("error page shows a Retry button", async ({ page }) => {
    ...
    const semanticsOk = await loadWithApiBlocked(page);
    if (!semanticsOk) test.skip(true, "Expo semantics unavailable");
    if (!(await isOnErrorPage(page, true)))
      test.skip(true, "Error page not triggered or not stable");

    // Retry button text comes from generalRetry = "Retry" in app_en.arb.
    await expect(page.getByText("Retry", { exact: true }).first()).toBeAttached(
      { timeout: 8_000 },
    );
  });
  ...
  test("clicking Retry triggers a retry attempt and button remains interactive", async ({
    page,
  }) => {
    test.setTimeout(45_000);
    const semanticsOk = await loadWithApiBlocked(page);
  ...
  test("error page does not log the user out (auth state preserved)", async ({
    page,
  }) => {
    test.setTimeout(45_000);
    const semanticsOk = await loadWithApiBlocked(page);
    if (!semanticsOk) test.skip(true, "Expo semantics unavailable");
    if (!(await isOnErrorPage(page)))
      test.skip(true, "Error page not triggered");
  ...
  test("dashboard pull-to-refresh with API blocked shows error gracefully", async ({
    page,
  }) => {
    ...
    // Skip if no CoC account — error-on-refresh only meaningful on authenticated shell.
    if ((await page.getByText("Home", { exact: true }).count()) === 0) {
      test.skip(
        true,
        "No CoC accounts — dashboard pull-to-refresh not testable",
      );
    }
