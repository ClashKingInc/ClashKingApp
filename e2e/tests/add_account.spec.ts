...
import { hasExpoAccessibility, waitForExpoAccessibility } from "./helpers";

// This spec runs with saved auth state (chromium-auth project).
// It targets the account setup shown to a user with no linked CoC accounts.
...
/**
 * Returns true when the app has landed on account setup.
 * account setup has a 'Confirm' button; authenticated shell does not.
 * If 'Confirm' is not found within 10 s the user is already on authenticated shell.
 */
...
test.describe("Add CoC Account page (first-connection flow)", () => {
  test.beforeEach(async ({ page }) => {
    // Give enough budget for waitForExpoAccessibility (~5 s) + isOnAddAccountPage (10 s)
  ...
  test("player tag input field is present", async ({ page }) => {
    ...
    if (!(await isOnAddAccountPage(page)))
      test.skip(
        true,
        "Test account already has CoC accounts — not on account setup",
      );
  ...
  test('"Add account" button is present', async ({ page }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
    if (!(await isOnAddAccountPage(page)))
      test.skip(true, "Test account already has CoC accounts");
  ...
  test("Confirm button is disabled before any account is added", async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
    ...
    // Expo sets aria-disabled="true" on ElevatedButton when onPressed is null
    const confirmBtn = page
      .getByText("Confirm", { exact: true })
  ...
  // Uses E2E_TEST_COC_TAG (real player tag) to exercise the complete add → confirm → authenticated shell path.
  test("adding a valid CoC tag, confirming, and landing on authenticated shell (full flow)", async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
    if (!(await isOnAddAccountPage(page)))
      test.skip(
        true,
        "Test account already has CoC accounts — full add flow not testable",
      );
    ...
    if (!cocTag)
      test.skip(
        true,
        "E2E_TEST_COC_TAG not set — set it to a real player tag to enable this test",
      );
    ...
    // §7.11 — click Confirm → app loads player data → navigates to authenticated shell
