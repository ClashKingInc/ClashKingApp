...
async function navigateToResetPage(page: any): Promise<boolean> {
  ...
  try {
    ...
    const forgotBtn = page
      ...
      .or(page.locator('[aria-label*="Forgot" i]'))
      .first();
    ...
    // Expo web exposes these strings via textContent, not aria-label.
    ...
    const onPage =
      (await page
        .locator('[aria-label*="Reset Code" i]')
...
test.describe("Reset password page", () => {
  ...
  test("page loads with Email, Reset Code, New Password and Confirm Password fields", async ({
    page,
  }) => {
    ...
    if (!reached)
      test.skip(
        true,
        "TEST_EMAIL not set or could not reach reset password page",
      );
    ...
    await expect(
      page
        ...
        .or(page.locator('[aria-label*="Reset Code" i]'))
        .first(),
    ).toBeAttached({ timeout: 5_000 });

    // New password field (type=password, labelled "New Password")
    await expect(page.getByLabel(/new password/i).first()).toBeAttached({
      timeout: 5_000,
    });
  ...
  test('"Reset Password" button is visible', async ({ page }) => {
    ...
    await expect(
      page
        ...
        .or(page.locator('[aria-label="Reset Password"]'))
        .first(),
    ).toBeAttached({ timeout: 8_000 });
  });

  test("submitting empty form stays on reset page (form validation)", async ({
    page,
  }) => {
  ...
  test("reset code shorter than 6 digits fails validation", async ({
    page,
  }) => {
    test.setTimeout(120_000);
    const reached = await navigateToResetPage(page);
  ...
  test("mismatched new passwords fail validation", async ({ page }) => {
    ...
    const mismatchError = page
      .locator('[aria-label*="do not match" i]')
      .or(page.locator('[aria-label*="mismatch" i]'));
  ...
  test("password that does not meet requirements fails validation", async ({
    page,
  }) => {
    test.setTimeout(120_000);
    const reached = await navigateToResetPage(page);
