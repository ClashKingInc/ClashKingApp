...
// Desktop Expo exposes Settings as an accessible sidebar button.
// All tests skip gracefully if the test account has no linked CoC account (not on authenticated shell).
//
// Settings tiles are accessible buttons. Section titles are text nodes in the
// React Native Web DOM, so the tests target their exact visible labels.
//
// Confirmation is exercised here without consuming the shared server session;
// the dedicated logout project performs the single destructive logout after all
// other authenticated specs complete.
...
const tile = (page: any, name: RegExp) =>
...
test.describe("Settings page", () => {
  ...
  test("settings page shows Preferences section (Language, Theme, Notifications)", async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
    ...
    await expect(
      page.getByText("Preferences", { exact: true }).first(),
    ).toBeAttached({ timeout: 8_000 });
  ...
  test("settings page shows Support section (FAQ, Discord, Translate)", async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
    ...
    await expect(
      page.getByText("Support", { exact: true }).first(),
    ).toBeAttached({ timeout: 8_000 });
  ...
  test("settings page shows About section (Licenses, Privacy Policy)", async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
    ...
    await expect(page.getByText("About", { exact: true }).first()).toBeAttached(
      { timeout: 8_000 },
    );
  ...
  test('clicking "Log out" tile opens a confirmation dialog', async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
  ...
  test("cancelling the logout dialog stays on settings page", async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
  ...
  test("settings page shows user email or username in the account section", async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
    ...
    // Expo shows the account username/email in the Account section.
    // The exact value (email vs username) is account-dependent, so assert that
    // the Account section title is present as proof the account block rendered.
    await expect(
  ...
  test("tapping Language tile opens a language selection dialog", async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
  ...
  // §14.7 — toggle theme
  test("tapping Toggle Theme switches the color scheme without crashing", async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
  ...
  test("tapping Notifications tile opens the notification settings screen", async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
  ...
  test('"Connected Accounts" tile is visible in the Account section', async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
    ...
    await expect(tile(page, /Connected Accounts/i)).toBeAttached({
      timeout: 8_000,
    });
  });

  // §16.2 — Connected Accounts page shows Discord and Email auth status tiles
  test('tapping "Connected Accounts" opens auth methods page with Discord and Email tiles', async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
    ...
    await expect(page.getByText(/Email/i).first()).toBeAttached({
      timeout: 5_000,
    });
  });
});
