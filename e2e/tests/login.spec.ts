...
test.describe("Login page — UI", () => {
  ...
  test("Email tab shows email input, password input and Login button", async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
    ...
    // Expo creates real <input> elements in the accessibility layer
  ...
  test("Forgot password and Sign up links are visible on Email tab", async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
    ...
    await expect(
      page
        ...
        .or(page.locator('[aria-label*="Sign" i]'))
        .first(),
    ).toBeAttached({ timeout: 8_000 });
  });
  ...
  test("full email login flow with test credentials", async ({ page }) => {
    ...
    if (loginResponse && loginResponse.status() >= 500) {
      test.skip(
        true,
        `Email login API unavailable (${loginResponse.status()})`,
      );
    }

    // Wait for the Login button to disappear → app navigated away from login page.
    // Expo web exposes the label via textContent, not aria-label; checking
  ...
  test("malformed email in login form is caught by client-side validation", async ({
    page,
  }) => {
    if (!(await hasExpoAccessibility(page)))
      test.skip(true, "Expo semantics unavailable");
