...
import { waitForExpo } from "./helpers";

test.describe("Smoke — app loads", () => {
  test("page returns 200 and Expo mounts", async ({ page }) => {
    const response = await page.goto("/");
    expect(response?.status()).toBeLessThan(400);
    await waitForExpo(page);
    // #root is Expo's React Native Web application mount.
  ...
  test("no JavaScript errors on load", async ({ page }) => {
    ...
    await page.goto("/");
    await waitForExpo(page);
    // Filter known Expo noise (font loading warnings etc.)
