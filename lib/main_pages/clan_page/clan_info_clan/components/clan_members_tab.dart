...
class ClanMembersState extends State<ClanMembers> {
  ...
  Widget build(BuildContext context) {
    Map<String, String> filterOptions = {
      ...
      AppLocalizations.of(context)?.townHallLevel ?? 'Town Hall Level': 'townHallLevel',
      AppLocalizations.of(context)?.trophies ?? 'Trophies': 'trophies',
      AppLocalizations.of(context)?.expLevel ?? 'Experience Level': 'expLevel',
      AppLocalizations.of(context)?.builderBaseTrophies ?? 'Builder Base Trophies': 'builderBaseTrophies',
      AppLocalizations.of(context)?.donations ?? 'Donations': 'donations',
      AppLocalizations.of(context)?.donationsReceived ?? 'Donations received': 'donationsReceived',
      AppLocalizations.of(context)?.donationsRatio ?? 'Donation Ratio': 'donationsRatio',
    };
...
// Define a map that assigns a weight to each role
    ...
    filteredMembers.sort((a, b) {
      ...
      switch (currentFilter) {
        case 'role':
          int roleComparison = (roleWeights[b.role] ?? 0).compareTo(roleWeights[a.role] ?? 0);
        ...
        case 'builderBaseTrophies':
          int builderBaseTrophiesComparison = b.builderBaseTrophies.compareTo(a.builderBaseTrophies);
        ...
        case 'donationsReceived':
          int donationsReceivedComparison = b.donationsReceived.compareTo(a.donationsReceived);
          return donationsReceivedComparison != 0
            ? donationsReceivedComparison
            : b.townHallLevel.compareTo(a.townHallLevel);
        case 'donationsRatio':
          int ratioComparison = ((b.donations / (b.donationsReceived == 0 ? 1 : b.donationsReceived)) * 1000).toInt()
            .compareTo(((a.donations / (a.donationsReceived == 0 ? 1 : a.donationsReceived)) * 1000).toInt());
        ...
        Stack(
          children: [ 
            ...
            Positioned(
              ...
              child:
              IconButton(
                icon: Icon(Icons.link, color: linkFilterActive ? Colors.green : Colors.grey),
                            ...
                            child: Row(
                              children: [
                                Column(
                                  children: [
                                    CachedNetworkImage(
                                      imageUrl: 'https://clashkingfiles.b-cdn.net/home-base/town-hall-pics/town-hall-${member.townHallLevel}.png',
                                      width: 40,
                                    ),
                                  ],
                                ),
                            ...
                            child: () {
                              switch (currentFilter) {
                                ...
                                case 'builderBaseTrophies':
                                  try {
                                    return Row(children: [
                                      SizedBox(width: 20),
                                      CachedNetworkImage(
                                        imageUrl: "https://clashkingfiles.b-cdn.net/icons/Icon_HV_Trophy.png",
                                        width: 24,
                                      ),
                                      SizedBox(width: 8),
                                      Text(
                                ...
                                case 'donations':
                                  return Row(children: [
                                    SizedBox(width: 20),
                                    Icon(LucideIcons.chevronUp, color: Color.fromARGB(255, 27, 114, 33)),
                                    SizedBox(width: 8),
                                    Text(
                                ...
                                case 'donationsReceived':
                                  return Row(children: [
                                    SizedBox(width: 20),
                                    Icon(LucideIcons.chevronDown, color: Color.fromARGB(255, 155, 4, 4)),
                                    SizedBox(width: 8),
                                    Text(
                                ...
                                case 'donationsRatio':
                                  double ratio = (member.donations / (member.donationsReceived == 0 ? 1 : member.donationsReceived));
                                  return Row(children: [
                                    SizedBox(width: 20),
                                    Icon(LucideIcons.chevronsUpDown, color: Color.fromARGB(255, 0, 136, 255)),
                                    SizedBox(width: 8),
                                    Text(
                                      (ratio) > 100
