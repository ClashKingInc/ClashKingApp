...
class WarInfo {
  ...
  factory WarInfo.fromJson(Map<String, dynamic>? json) {
    try {
      ...
      return WarInfo(
        tag: data['war_tag']?.toString(),
        state: data['state']?.toString() ?? 'unknown',
        teamSize: (data['teamSize'] as num?)?.toInt(),
        attacksPerMember: (data['attacksPerMember'] as num?)?.toInt(),
        ...
        clan: _asMap(data['clan']) != null
            ? WarClan.fromJson(_asMap(data['clan']))
            : null,
        opponent: _asMap(data['opponent']) != null
            ? WarClan.fromJson(_asMap(data['opponent']))
            : null,
      );
    } catch (e) {
    ...
    if (clan?.tag == clanTag) {
      if (state == 'warEnded') {
        ...
        if (clan!.stars > opponent!.stars) {
          ...
          if ((clan!.destructionPercentage) >
              (opponent!.destructionPercentage)) {
            return 'won';
          } else if ((clan!.destructionPercentage) <
              (opponent!.destructionPercentage)) {
            return 'lost';
          } else {
            return 'tie';
      ...
      if (state == 'warEnded') {
        ...
        if (opponent!.stars > clan!.stars) {
          ...
          // Tie on stars, check destructionPercentage
          if ((opponent!.destructionPercentage) >
              (clan!.destructionPercentage)) {
            return 'won';
          } else if ((opponent!.destructionPercentage) <
              (clan!.destructionPercentage)) {
            return 'lost';
          } else {
            return 'tie';
  ...
  WarInfo reorderForUser(String userPlayerTag) {
    ...
    final isUserInClan =
        clan?.members.any((member) => member.tag == userPlayerTag) ?? false;
