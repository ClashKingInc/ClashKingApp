...
class LegendSeason extends StatelessWidget {
  ...
  const LegendSeason({super.key, required this.player, required this.season});
        ...
        child: Card(
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                Text(
                  AppLocalizations.of(context)!.statsSeasonStats,
        ...
        for (int star = 3; star >= 0; star--)
          Row(
            children: [
              ...
              const SizedBox(width: 4),
              Text(
                "${percentages[star]?.toStringAsFixed(1) ?? '0.0'}% (${attacksdefs[star] ?? 0})",
                style: Theme.of(context).textTheme.bodyMedium,
              ),
            ],
          ),
