...
class PerformanceAnalysisService {
  ...
  static List<FilterPreset> analyzePerformance(
    ...
    if (stats.averageStars < 2.0) {
      suggestions.add(
        _createPreset(
          'failed-attacks',
          localizations?.performanceAnalysisFailedAttacks ??
              'Failed Attacks (0-1 Stars)',
          localizations?.performanceAnalysisFailedAttacksDesc ??
              'Shows attacks that failed to get 2+ stars',
          WarStatsFilter(allowedStars: [0, 1]),
        ),
      );
    }
    ...
    final perfectRate = stats.totalAttacks > 0
        ? threeStarCount / stats.totalAttacks
        : 0.0;
    if (perfectRate < _lowSuccessRateThreshold) {
      suggestions.add(
        _createPreset(
          'missed-perfects',
          localizations?.performanceAnalysisMissedPerfects ??
              'Missed Perfect Attacks',
          localizations?.performanceAnalysisMissedPerfectsDesc ??
              'Shows attacks that got 2 stars - potential perfect opportunities',
          WarStatsFilter(allowedStars: [2]),
        ),
      );
    }
        ...
        if (enemyTh != null && thData.count >= 3 && thData.averageStars < 1.5) {
          suggestions.add(
            _createPreset(
              ...
              localizations?.performanceAnalysisThAttackIssues(enemyTh) ??
                  'TH$enemyTh Attack Issues',
              localizations?.performanceAnalysisThAttackIssuesDesc(enemyTh) ??
                  'Shows poor performance against TH$enemyTh',
              WarStatsFilter(enemyTownHalls: [enemyTh]),
            ),
          );
        }
      }
    ...
    for (final entry in defStats.entries) {
      ...
      if (parts.length >= 2) {
        ...
        if (attackerTh != null &&
            defData.count >= 3 &&
            defData.averageStars > 2.0) {
          suggestions.add(
            _createPreset(
              'th$attackerTh-defense-weak',
              localizations?.performanceAnalysisThDefenseIssues(attackerTh) ??
                  'TH$attackerTh Defense Issues',
              localizations?.performanceAnalysisThDefenseIssuesDesc(
                    attackerTh,
                  ) ??
                  'Shows where you\'re vulnerable to TH$attackerTh attacks',
              WarStatsFilter(enemyTownHalls: [attackerTh]),
            ),
          );
        }
      }
    ...
    if (cwlStats.warsCounts >= 3 && randomStats.warsCounts >= 3) {
      ...
      if (cwlSuccess < randomSuccess - 0.5) {
        suggestions.add(
          _createPreset(
            'cwl-issues',
            localizations?.performanceAnalysisCwlIssues ??
                'CWL Performance Issues',
            localizations?.performanceAnalysisCwlIssuesDesc ??
                'Shows CWL attacks - lower success than regular wars',
            WarStatsFilter(warTypes: ['cwl']),
          ),
        );
      }
      ...
      if (randomSuccess < cwlSuccess - 0.5) {
        suggestions.add(
          _createPreset(
            'random-war-issues',
            localizations?.performanceAnalysisRandomWarIssues ??
                'Random War Issues',
            localizations?.performanceAnalysisRandomWarIssuesDesc ??
                'Shows random war attacks - lower success than CWL',
            WarStatsFilter(warTypes: ['random']),
          ),
        );
      }
    }
    ...
    suggestions.add(
      _createPreset(
        ...
        localizations?.performanceAnalysisFreshOnlyDesc ??
            'Analyzes performance on fresh bases only',
        WarStatsFilter(freshAttacksOnly: true),
      ),
    );
    ...
    suggestions.add(
      _createPreset(
        'recent-performance',
        localizations?.performanceAnalysisRecentPerformance ??
            'Recent Performance (30 Days)',
        localizations?.performanceAnalysisRecentPerformanceDesc ??
            'Shows recent performance to identify current trends',
        WarStatsFilter(startDate: recentDate),
      ),
    );
    ...
    suggestions.add(
      _createPreset(
        ...
        localizations?.performanceAnalysisHighStakesDesc ??
            'Shows attacks from top 5 war positions',
        WarStatsFilter(maxMapPosition: 5),
      ),
    );
    ...
    suggestions.add(
      _createPreset(
        ...
        localizations?.performanceAnalysisCleanupCrewDesc ??
            'Shows attacks from lower war positions',
        WarStatsFilter(minMapPosition: 6),
      ),
    );
