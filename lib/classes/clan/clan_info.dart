...
class ClanService {
  ...
  Future<Clan> fetchClanInfo(Clan clanInfo) async {
    try {
      ...
      Sentry.captureException(exception, stackTrace: stackTrace);
      Sentry.captureMessage(
          'Failed to load clan stats, clanInfo: ${clanInfo.toString()}');
      throw Exception('Failed to load clan stats: $exception');
    }
  }
  ...
  Future<WarStateInfo> fetchCurrentWarInfo(String clanTag, bool bypass) async {
    try {
      ...
      Sentry.captureMessage(
          'Failed to load current war info, clanTag: $clanTag');
      return WarStateInfo(state: "notInWar");
    }
  }
  ...
  Future<CurrentLeagueInfo?> fetchCurrentLeagueInfo(String clanTag) async {
    try {
      return await retry(
        () async {
          final responseCwl = await http
              .get(
                Uri.parse(
                    'https://api.clashk.ing/v1/clans/${clanTag.replaceAll('#', '%23')}/currentwar/leaguegroup'),
              )
              .timeout(Duration(seconds: 15));
      ...
      Sentry.captureException(e, stackTrace: stackTrace);
      Sentry.captureMessage(
          'Failed to load current league info, clanTag: $clanTag');
      return null;
    }
  }
}
