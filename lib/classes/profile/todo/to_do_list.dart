...
class ToDoList {
  ...
  late int totalLegends;
  late int totalSeasonPass;
  late int totalRaidsAttacks;
  late int totalCwlAttacks;
  late int totalClanGamesPoints;
  late int numberAccounts;
  late int totalWarAttacks;
  ...
  void calculateTotals() {
    try {
      ...
      totalCwlAttacks =
          items.fold(0, (sum, item) => sum + item.cwl.attacksDone);
  ...
  static Future<ToDoList> fromJson(Map<String, dynamic> json) async {
    try {
      ...
      Sentry.captureException(exception, stackTrace: stackTrace);
      Sentry.captureMessage('Failed to parse to-do list from json, json: $json');
  ...
  ToDo? findTodoByTag(String tag) {
    try {
      ...
      Sentry.captureException(exception, stackTrace: stackTrace);
      Sentry.captureMessage('No to-do found for this tag, tag: $tag');
      return null;
    }
  }

  void deleteToDoByTag(String tag) {
    items.removeWhere((todo) => todo.playerTag == tag);
  }
