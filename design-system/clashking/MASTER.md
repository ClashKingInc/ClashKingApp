...
Emerging but not yet centralized — document here until `AppRadius` (in `lib/common/theme/app_tokens.dart`) exists and can be referenced directly:
...
| Card/panel radius | `28px` | `CardTheme`, hero header stats panels (`LiquidGlassBar`) |

### Spacing

No centralized spacing token class exists yet — padding/gaps are inline magic numbers (commonly 8/12/14/16/18/20/24, no strict system). Not a blocker, but a candidate for future cleanup; don't invent a different scale ad hoc when touching new screens — reuse the values already common in the surrounding file.
...
`lib/common/widgets/liquid_glass.dart` — `LiquidGlassBar`: renders the shared `liquid_glass_widgets` shader inside Flutter's compositor on every platform. Used for header stats panels, floating action buttons, segmented controls, and tab bars. Keeping glass out of UIKit platform views is required for tear-free iOS route gestures.
...
**Style:** Material 3 + Flutter-composited Liquid Glass, applied through a "hero header" pattern.
...
- ❌ **Emojis as icons** — use Material/vector icons. Live violations should be fixed rather than repeated.
...
- [ ] No emojis used as icons (use Material/vector icons)
...
- [ ] New floating surfaces use the glass/hero pattern (`LiquidGlassBar`), not a flat `Card`, unless there's a specific reason to diverge
