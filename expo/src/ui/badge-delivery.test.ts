...
test.each([
  ...
  const url = sizedBadgeUrl(
    'https://badges.clashk.ing/ABC?token=ignored&size=small',
    points,
    points,
    scale,
  );
...
test.each(['small', 'medium', 'large', '64', '128', '256', '512'])(
  'PNG fallback preserves allowed size %s',
  (size) => {
    expect(originalBadgeUrl(`https://badges.clashk.ing/ABC.avif?size=${size}&hint=ignored`)).toBe(
      `https://badges.clashk.ing/ABC.png?size=${size}`,
    );
  },
);
test('widget and unmeasured fallbacks always have explicit extensions', () => {
  ...
  expect(originalBadgeUrl('https://badges.clashk.ing/ABC')).toBe(
    'https://badges.clashk.ing/ABC.png?size=512',
  );
  expect(originalBadgeUrl('https://badges.clashk.ing/ABC?size=1024')).toBe(
    'https://badges.clashk.ing/ABC.png?size=512',
  );
