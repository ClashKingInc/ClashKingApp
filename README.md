...
`CK_API_V2_BASE_URL=https://api.clashk.ing/v2`.
...
Pass `--dart-define=CK_API_ENV=development` or
`--dart-define=CK_API_ENV=staging` to `flutter run` to use
...
production uses `https://api.clashk.ing/v2` for app requests and
`https://api.clashk.ing/proxy/v1` for proxied Clash API requests.
