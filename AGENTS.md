...
- Local development uses the production API at `https://api.clashk.ing` but keeps `https://dev-app.clashk.ing/auth/discord_callback.html` as its Discord redirect. Cloudflare Pages staging uses `https://staging-app.clashk.ing` and its matching callback instead.
