...
Before committing any app change, inspect every supported `lib/l10n/*.arb` file rather than checking only the template locale or relying on runtime fallback.
...
- Run the repository's localization generation command and inspect `untranslated_messages.json`. No key added or changed by the task may remain untranslated, and tracked generated localization output must be current.
