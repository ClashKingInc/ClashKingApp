...
The shipping app is the Expo SDK 57 project under `expo/`. Run application commands from that directory with Node.js 22; the root Flutter tree is a frozen behavioral, asset, and localization reference.
...
- Feature services preserve the frozen Flutter ownership boundaries for accounts, bookmarks, players, clans, wars, and widgets.
...
- Flutter files under `lib`, `ios`, `android`, and `web` are reference inputs only. New shipping behavior belongs under `expo`.
...
- Add localization keys to every supported `expo/src/i18n/arb/*.arb` file, regenerate catalogs, and pass localization parity checks.
